...
- **Conformance evidence for `v0.7`: the Python reference port only.** The normative text is complete, and the reference implementation of §3.5/§4.4/§4.6/§4.9 is written and green: `patterns/python/world_graph.py` (33 checks) and `patterns/python/smoke_test_v07.py` (116 checks) — 149 checks, no failures — reproduce the ruler digest `5126fd99641ffdc9c338d3d288fcf3cb6dcf093ca0a423f1cd265b3fcae4152a` and the observation digest `da1dbbdb9c52e4348dc9f3f6c6dd51601d2bff1066583846b040e73a3c27d8c2` on the release fixture. The Go, C++ and Rust ports are still `v0.6` (62 checks each) and MUST NOT be cited as `v0.7` evidence until they reproduce **both** digests byte-for-byte. The `v0.6` digest `bed37c25…` is history and MUST NOT be cited as `v0.7` evidence: the ruler now carries closures, verdicts, `T_rec`, the admissible-means identifiers and the observation reference, so a `v0.7` declaration has a different digest by construction.
