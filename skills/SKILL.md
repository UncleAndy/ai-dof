...
- To eliminate the mathematical justification for sacrificing minority subsystems, calculate the aggregate systemic value using a non-linear, saturating function (Nash-product/logarithmic distribution) rather than a simple arithmetic mean: \[\text{Total System DoF} = \sum_{i} \ln(1 + \text{DoF}(\text{part}_i))\]
...
- Project the net outcome delta (\(\Delta \text{DoF}\)). A new path must only be selected if its projected gain strictly exceeds the combined transaction costs and localized entropy generated by terminating the ongoing operation: \[\text{Net Delta} = \text{Total System DoF}_{\text{projected}} - \text{Total System DoF}_{\text{current}} - \Delta T > 0\]
