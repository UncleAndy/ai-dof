...
- **The observation MUST be arbitrage-free.** An axis rate is the best product of quotes along a path (§4.8), so a cycle whose product exceeds `1` makes the rate unbounded and defeats the resource gate. Such an observation is **non-conformant**: the affected rate is `undetermined` and the exchange does not happen. A favourable quote is not a licence.
...
- **Determinism and canonical ties.** The cheapest path is selected by its canonically quantized value, then by fewer edges, then lexicographically by edge identifier (§3.4.3), so that two conformant implementations report the same witness for the same observation.
