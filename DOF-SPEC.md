...
- **Conformance evidence for `v0.7`: the Python and Go reference ports.** The normative text is complete, and two implementations of §3.5/§4.4/§4.6/§4.9 reproduce the release's fingerprints on the release fixture, byte for byte:
  - **Python** — `patterns/python/world_graph.py` (33 checks) + `patterns/python/smoke_test_v07.py` (116 checks) = 149 checks, no failures.
  - **Go** — `patterns/go/world_graph.go`, `fixture_v07.go`, `options_v07.go`, `harness_v07.go` (116 checks), with the canonical declaration text verified **identical** to the Python one by `go run . dump` — the tool that turns a cross-port mismatch into a diff instead of a mystery.
  Both produce the ruler digest `5126fd99641ffdc9c338d3d288fcf3cb6dcf093ca0a423f1cd265b3fcae4152a` and the observation digest `f3891c6ab622325fd6668893dd9f7450d39aa2d0a7ad2849634a4f59219f6a1c`, and so does the release fixture's index (`-35.314438370902`). The C++ and Rust ports are still `v0.6` (62 checks each) and MUST NOT be cited as `v0.7` evidence until they reproduce **both** digests. The `v0.6` digest `bed37c25…` is history and MUST NOT be cited as `v0.7` evidence: the ruler now carries closures, verdicts, `T_rec`, the admissible-means identifiers and the observation reference, so a `v0.7` declaration has a different digest by construction.
  - A portability observation found while the second port landed: the evaluation index is a sum of logarithms over a **set**, so two ports that iterate their container in different orders can differ in the last bits (~1e-15) while agreeing on every derivation. Every comparison of an index therefore MUST use a tolerance, and any future *hash* of an index would have to fix a canonical summation order first — the same rule §3.4.3 already applies to the digest.
