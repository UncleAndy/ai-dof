...
3. **Слой Валидации (Calculus Core):**
   - **Задача:** Принимает планы от Генератора. Проводит симуляцию для каждого. Фильтрует их через нелинейную формулу $\sum \ln(\text{DoF})$. Блокирует любые варианты со штрафом $-\infty$.
