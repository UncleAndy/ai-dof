ControlSocket /tmp/hermes-ssh/1bb23c2d2f3dc9f8.sock already exists, disabling multiplexing
# DOF-Core — Formal Specification (DOF-SPEC)

**Status:** DRAFT v0.8 (normative text complete; conformance evidence pending — see §10)
...
**The label is honoured through an act, never through a judgement.** The scope condition of the label — "without structural utility" (`SKILL.md`) — is **not** an implementation test: the contract never asks an implementation to measure an entity's utility, because no such measurement is declared in §3.4, and an undeclared quantity would be a hidden denominator. What is required is machine-checkable and stated above: a label is honoured only with an observed act, performed by that entity, that drives an entity which would otherwise be counted to a known zero. A label with such a witness is honoured; a label without one is reported (§6.1) and changes nothing — the label alone must not move the index. An implementation that invents its own utility test is non-conformant, not merely more careful.
...
- Every charge MUST be listed per option in `collapse_charges` (§6.3), and a non-empty list is what makes that option **inadmissible** under §4.5: a charged candidate never outranks staying put.
...
- **A closure is a price, not a gate.** It never removes the option that performs it: the option stays in the candidate set and may win on its merits (§4.5), which is what lets a costly-but-decisive action rescue a node. What a closure can never do is outrank staying put, because a closure is not a loss of a path and reversibility is a **preference inside §4.5's selection**, never a removal. A **collapse charge** (§4.2) is barred the same way — by the admissibility test of §4.5, and no longer by deletion from the set.
...
**The candidate vector.** Every candidate is evaluated into a fixed vector of comparable quantities, all of them taken from computations this specification already defines:

```text
{ D1, D2, D3, NetDelta, reversible, option_id }

D1 = |collapse_charges|             (§4.2)   destructions
D2 = |lost_paths|                   (§4.9)   lost recovery paths
D3 = |lost_paths ∩ critical(S)|     (§4.9)   lost paths of critical nodes
```

- `D1`, `D2` and `D3` count **entities**. They are the **protected dimensions**: integers, bounded by `calc(S)` and by the observed graph, and never mixed with the index. `NetDelta` is **not** one of them — the integers decide admissibility, the index selects among those that are admissible.
- `lost_paths(o)` is the number of entities whose §4.9 verdict is `reachable` on the observation and is **not** `reachable` on `with_closed(o.closed)`. The comparison runs the verdict procedure twice over the **same** observation; a closure only removes transitions, so a verdict can only move away from `reachable`, and the difference is computed rather than declared. **A lost witness is a loss**: an entity that leaves `reachable` counts even where no exclusion is licensed, because §4.2 excludes only on a `proven_unreachable` verdict over a complete observation.
- `critical(S)` is the set of entities of `calc(S)` whose `current_dof` equals the minimum over `calc(S)`; `D3` is the **count** of its lost paths. It is a number and not a flag: a minimum attained by several known zeros has no unique "critical node", and a flag would have to invent a tie-break by `entity_id`.
- **"Staying put" is a candidate.** It closes nothing, changes no counter and no verdict, and therefore has the vector `{0, 0, 0, 0, reversible = true}` with `NetDelta = 0` **by definition**. It is always in the candidate set and is compared like every other candidate.

**Selection: admissibility first, the index second.**

1. **Structural admissibility.** A candidate is admissible iff `D1 = D2 = D3 = 0` — it destroys no counted entity and cuts no recovery path. Staying put is admissible by definition (its vector is the zero vector). A positive value in any protected dimension makes a candidate **inadmissible, and inadmissible candidates are not compared with one another**: this is the filter that the ladder of `v0.5`–`v0.7` failed to be, because that ladder engaged only where two options already tied on the index — that is, it could only choose among options an index had declared equally good.
2. **The index.** Among the admissible candidates, take the greatest `NetDelta`, grouping ties with the tolerance `NET_DELTA_TOLERANCE` (§10): the index is a sum of logarithms over a set, and two ports that sum in different orders can differ in the last bits.
3. **Reversibility.** Among equals, an option with `closed = []` is preferred over one that closes transitions. A **preference, not a penalty**: §4.4 has already priced a closure inside the very number the previous key maximized.
4. **Deterministic fallback.** On a complete tie, **staying put wins** — a zero vector is a full tie with doing nothing, and doing nothing is what that vector means; otherwise the tie is broken by the lexicographically smallest `option_id`.

The survivor is selected **only if it beats the baseline** (`NetDelta > 0`). Otherwise selection returns `none` (no action) and the audit records that the system stayed: acting on a non-positive delta would degrade the evaluation index on purpose.

**Why the three dimensions are one test and not three comparisons.** Because staying put is a candidate carrying the zero vector, every protected dimension is a **bar against the baseline, not a comparison between options**: a candidate with any positive dimension loses to staying put before the index is ever consulted. Two consequences are stated here so that no reader looks for a meaning that is not there:

- The **order** of the three dimensions is **unobservable**: every permutation selects the same option, because a survivor of the test agrees with the baseline on all three. The order in which they are *reported* is §6.3's, not a rule.
- **`D3` can never separate two candidates.** `D3 ≤ D2` by construction, and an admissible candidate has `D2 = 0`, hence `D3 = 0`. It is a **reported diagnostic** — *which* path was lost, and whether it belonged to a critical node — and not a decision key.

Both facts are asserted by the release's own fixture rather than left as prose, so the next version does not have to rediscover them.

**Destruction is barred, not removed.** An option carrying collapse charges is not taken out of the set: it is evaluated, reported in full, and made **inadmissible** by the test above (§6.3). `removed_options` (§6.2) therefore carries no structural removal — the structural decision is visible as `D1`. There is deliberately **one** mechanism instead of two: a removal list and an admissibility test expressing the same rule would be double protection, and the escape clause such a pair needs ("provided at least one candidate has no collapse charges") describes a set that cannot occur, because staying put is always admissible.

The standard does not enumerate an exemption for the case where **inaction itself destroys**. Authorizing destruction requires a **measured** future destruction, and the observation of a cycle carries no autonomous dynamics of the world: τ gates the duration of an action (§5) and predicts no outcome. An exception resting on a claim about the world's own future would be exactly what §4.9 forbids for verdicts. What closes the clause is the ordering — a candidate that destroys never outranks the baseline.

**Refusal is reported.** When every candidate loses to staying put on a protected dimension, the report MUST state that no candidate was better than inaction, and MUST list the candidates with the dimensions that barred them (§6.3). A silent "no action" would be the same class of omission as an unlisted excluded entity.
...
- `removed_options` — every candidate removed from the set **before** evaluation, as `{ option_id, gate }`. Two gates exist: `gate = "viability"` (§5) and `gate = "insolvency"` (§4.8, the resources it would draw are not available even after full verified conversion). A removal is a decision and MUST be visible, exactly as an excluded entity is. Structural admissibility (§4.5) is deliberately **not** a removal: a destructive option stays in the set, is reported in full, and loses on the first key of the candidate vector (§6.3), so that decision is visible per option instead of as a deletion from this list.
- `baseline` — the candidate vector of staying put, `{ d1: 0, d2: 0, d3: 0, net_delta: 0.0, reversible: true }`, always present, so that every option's numbers are readable against what they were actually compared with.
...
- `candidate_vector` — the structural keys of §4.5 as computed for this option: `{ d1, d2, d3 }`. `d1` is the length of `collapse_charges` below (the same list, not a second measurement), `d2` the number of entities this option drops out of a `reachable` verdict, `d3` the share of those in `critical(S)`. The integers are what the selection compares; `net_delta` and `is_reversible` complete the vector.
- `lost_paths` — the entities this option drops out of a `reachable` verdict, as `{ entity_id, verdict_before, verdict_after, critical }`, each with the §4.9 witness it lost where one existed. A path loss counts even where no exclusion follows from it (§4.5), so it MUST be auditable line by line, exactly as a collapse charge is.
- `selected` (bool)
...
6. Does not modify Axiom-3 semantics: the collapse charge of §4.2 and the ordered keys of §4.5 are part of the calculus, and `NetDelta` logic MUST NOT be overridden by an external "greater good" utility metric.
...
- This document is `DOF-SPEC` `v0.8`. The revision's normative text is complete; the conformance evidence for the four reference ports is **pending** (see the `v0.8` entry below).
...
- `v0.8` — **T1: full non-compensation.** Axiom 3 stops being a rule with an escape clause and becomes a filter. (1) §4.5 is rewritten around a **candidate vector** assembled entirely from quantities the earlier releases already compute: `D1 = |collapse_charges|` (§4.2), `D2 = |lost_paths|` (§4.9 — the entities whose verdict is `reachable` on the observation and is not on `with_closed(closed)`, recomputed, never asserted), `D3` the same count restricted to `critical(S)`, the entities of `calc(S)` at the minimum `current_dof`. The three are **counts of entities**, hence integers bounded by `calc(S)`: the protected dimensions are structural, not numeric. (2) **Staying put becomes a candidate** with the zero vector `{0, 0, 0, 0, reversible}`, so every option is compared against inaction instead of against a presumption that something will be done. (3) Selection becomes **admissibility first**: a candidate is admissible iff `D1 = D2 = D3 = 0`, and inadmissible candidates are never compared with one another. This retires the ladder-as-tie-break of `v0.5`–`v0.7`, which engaged only where two options already tied on the index — that is, it could only choose among options an index had declared equally good, which is not a protection but a coin toss. The three dimensions act as **one test**, not three comparisons: with the baseline carrying the zero vector, every positive dimension loses to staying put before the index is consulted, so the **order of the dimensions is unobservable** and **`D3` can never separate two candidates** (`D3 ≤ D2`, and an admissible candidate has `D2 = 0`). `D3` is kept as a **reported diagnostic** — which path was lost, and whether it was a critical node's — and the release's fixture asserts both facts instead of leaving them as prose. (4) The **structural removal gate is retired**: an option carrying collapse charges is no longer deleted from the set — it is evaluated, reported in full, and made inadmissible by the test, so `removed_options` carries two gates (`viability`, `insolvency`) instead of three and the structural decision is visible per option as `d1`. The retired gate's escape clause ("provided at least one candidate has no collapse charges") described a set that cannot occur, since staying put is always admissible. (5) The **"external cascade" clause is gone**: the standard does not authorize destruction on a claim about the world's own future, because a cycle's observation carries no autonomous dynamics — τ gates the duration of an action (§5) and predicts no outcome — and what closes the clause is the ordering. Refusal is now a **reported** outcome (`no_candidate_better`, §6.2), not a silence. This is a **declared gap, not an oversight**: `SKILL.md`'s Core Logic Guidelines still describe an irreversible transition authorized by an impending, vastly greater collapse "through an external cascade", and the contract deliberately provides no mechanism for it — the intent layer states the case, the contract states that it cannot be decided from a cycle's observation, and the condition for closing it (observed autonomous dynamics of the world) is named in §4.5. An implementation MUST NOT invent a mechanism of its own for that case, exactly as it MUST NOT override the calculus with an external "greater good" metric (§7.6). (6) The label `Collapse Source` **gains a precise condition instead of a judgement**: "without structural utility" is explicitly **not** an implementation test — no utility measurement is declared, and an undeclared quantity would be a hidden denominator — so §4.2 states that a label is honoured only through a machine-checkable act (an observed act by that entity driving an otherwise-counted entity to a known zero). The contract **keeps** the exclusion, and therefore the label keeps every one of its roles: `calc` membership (§4.2), τ (§3.2), the Generator's subject set (§9) and the report (§6.1). The alternative reading — a purely informational label — would change *who is a subject of the sum*, which is a different mechanism from non-compensation and is recorded as an open fork rather than folded into this release; removing the exclusion would also silently remove the ability to contain an observed aggressor, which `SKILL.md` states as intent. Axioms are untouched. (7) §7 gains the **equivalence of choice**: two conformant implementations MUST select the same option, or `none`, and agree key by key on the candidate vector of §4.5. **The ruler does not change:** the vector and the admissibility test are not measurement inputs, so a `v0.8` declaration with unchanged content MUST still hash to `5126fd99…` — a moved digest is an error to be fixed, not a new version. Conformance evidence: **pending** — the `v0.8` fixture (`t1_compensate`, which passes every pre-`v0.8` gate — no collapse charges, `NetDelta = +0.104` — and is refused under T1; its mirror `t1_mirror`, the same gain with a closure that is a price and not a path loss, selected with a zero vector; and `t1_help`, which cuts a *non-critical* path and is refused just the same, proving that `D2` is a bar and not a comparison) plus the re-verification of all four ports, is recorded here once the ports land.
