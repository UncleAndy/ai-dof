...
**3.4.3 Canonical serialization.** A digest is comparable only if the declaration is serialized canonically: fixed key order, fixed numeric formatting, no free-form prose inside hashed content. Two implementations that measure the same state with the same procedure MUST produce equal digests.
...
- `lens_terms` — one entry per lens of the frozen set (§4.6): `{ lens, psi, dof_known, contribution }`. The entries' `contribution` MUST sum to `contribution`, so the report shows *why* the value is what it is rather than only *what* it is.
