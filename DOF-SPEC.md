...
- Every charge MUST be listed per option in `collapse_charges` (§6.3), and a non-empty list is what bars that option on the first key of §4.5: a charged candidate never outranks staying put.
...
- **A closure is a price, not a gate.** It never removes the option that performs it: the option stays in the candidate set and may win on its merits (§4.5), which is what lets a costly-but-decisive action rescue a node. What a closure can never do is outrank staying put — reversibility is one of the structural keys of §4.5, not a removal. A **collapse charge** (§4.2) is barred the same way, by the first key of §4.5, and no longer by deletion from the set.
...
**Selection: the keys are an ordered filter, not a tie-break.** Each key is applied only to the candidates surviving the previous ones:

1. **Destructions** — the fewest `D1` (Axiom 3).
2. **Lost paths** — the fewest `D2`.
3. **Critical paths** — the fewest `D3`.
4. **`NetDelta`** — the greatest. The survivor is selected **only if `NetDelta > 0`**: acting on a non-positive delta would degrade the evaluation index on purpose, so a survivor that does not beat staying put returns `none` (no action) and the audit records that the system stayed.
5. **Reversibility** — an option with `closed = []` is preferred over one that closes transitions. This is a **preference, not a penalty**: reversibility is the weakest structural key, because §4.4 has already priced what a closure costs in the very number the previous key maximized.
6. **Deterministic fallback** — on a complete tie, **staying put wins** if it is still a candidate: a zero vector is a full tie with doing nothing, and doing nothing is what that vector means (key 4 already returns `none` for it). Otherwise the tie is broken by the lexicographically smallest `option_id`.

**Destruction is barred, not removed.** An option carrying collapse charges is not taken out of the set: it is evaluated, reported in full, and loses to staying put on the first key (§6.3). `removed_options` (§6.2) therefore carries no structural removal — the structural decision is visible as `D1`. There is deliberately **one** mechanism instead of two: a removal list and a first key expressing the same rule would be double protection, and the escape clause such a pair needs ("provided at least one candidate has no collapse charges") describes a set that cannot occur, because staying put is always a charge-free candidate.
...
10. Applies the **frozen calculation set** of §4.2 — `calc` is computed on `S` and the same entities are summed in every `S'`, so a counted entity driven to a known zero is charged the floor `ln ε` instead of disappearing — computes the **candidate vector** of §4.5 (with `d2`/`d3` recomputed by its own verdict procedure on `with_closed(closed)`, never asserted), applies the keys as an **ordered filter** so that a structural key outranks the index, treats staying put as a candidate with the zero vector, and reports the vector of every option plus the baseline. No candidate is removed for structural reasons: destruction loses on the first key instead of leaving the set. Viability and insolvency removals are listed in `removed_options`.
