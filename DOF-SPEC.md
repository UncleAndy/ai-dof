...
**3.4.2 Levels.** *State-level* (identical for all options of one cycle): the ordered lens set, the acting agent's means per resource **with their units**, the derived groups, the observed rates and the declared mandate, `τ`, the `u₀` prior, `M(S)`, procedure version. *Entity-level*: `V` and `V_env`, `F` and `F_env`, `T_rec`, the durations.
...
- **Known limitation (reserved).** A flat nats penalty weighs differently depending on the size of `calc(S')`, and it is a declared constant rather than a quantity derived from the entity's own cost profile. Expressing rigidity in the cost units of §4.6 (`x_rig`) is a reserved change. The prerequisites this clause used to name — the agent's means in the state and a declared draw per option — are **normative since `v0.6`** (§3.2 `resources`, §3.3 `projected_resource_delta`); what remains missing is the definition of `x_rig` itself. Until then the value `0.5` MUST be used.
