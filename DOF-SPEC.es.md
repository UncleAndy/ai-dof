...
| `is_entropy_source`  | bool    | —                         | Si `true`, la entidad es un agresor destructivo (véase §4.2). |
...
Una entidad con `current_dof == 0.0` está en colapso (véase §4.1).
...
- `entity_id`
- `is_entropy_source`
- `included_in_sum` (bool) — `false` syss si `is_entropy_source`
- `current_dof`
